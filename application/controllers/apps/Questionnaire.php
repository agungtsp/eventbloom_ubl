...
class Questionnaire extends CI_Controller {
	...
	function index(){
		render('apps/questionnaire/index',$data,'apps');
	}
	public function add($id=''){
		if($id){
			$data = $this->QuestionnaireModel->findById($id);
			if(!$data){
				die('404');
			}
			$data['judul']        = 'Edit';
		...
		render('apps/questionnaire/add',$data,'apps');
	}
	function records(){
		$data = $this->QuestionnaireModel->records();
		foreach ($data['data'] as $key => $value) {
		}
		render('apps/questionnaire/records',$data,'blank');
	}	
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->db->trans_start();   
				...
				if($idedit){
					...
					$this->QuestionnaireModel->update($post,$idedit);
				}
				else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert RefEventCategory";
					$this->QuestionnaireModel->insert($post);
				}
				detail_log();
				insert_log($act);
